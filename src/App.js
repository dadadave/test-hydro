...
.fx-zone-btns{display:flex;gap:6px;flex-wrap:wrap;}
.fzb{font-family:var(--mono);font-size:9px;font-weight:700;letter-spacing:1px;
  padding:6px 12px;border:2px solid var(--border);border-radius:3px;
  cursor:pointer;background:var(--bg3);color:var(--muted);
  transition:all .15s;text-transform:uppercase;white-space:nowrap;user-select:none;
  -webkit-tap-highlight-color:transparent;}
.fzb:hover{border-color:var(--light);color:var(--text);}
.fzb.sel-COL {background:rgba(239,68,68,.18);border-color:#ef4444;color:#ef4444;}
...
.fx-note{background:var(--bg3);border:1px solid var(--border);border-radius:3px;
  ...
  outline:none;transition:border .15s;flex:1;min-width:120px;}
.fx-note:focus{border-color:var(--amber);}
.fx-note::placeholder{color:var(--muted);}
.fx-op{background:var(--bg3);border:1px solid var(--border);border-radius:3px;
  color:var(--amber2);font-family:var(--mono);font-size:10px;font-weight:700;
  ...
  const saveCorrection = async (b, statut) => {
    ...
    try {
      const payload = {
        bouteille_id:    b.id,
        lot_id:          b.lot_id,
        session_id:      b.lots?.session_id || sess.id,
        num_serie:       b.num_serie,
        machine:         sess.machine || "",
        bottle_type:     sess.bottle_type || "",
        operateur:       sess.operateur || null,
        date_test:       sess.date || new Date().toISOString().split("T")[0],
        zone_col:        b.zone_col,
        zone_med:        b.zone_med,
        zone_gal:        b.zone_gal,
        zone_pied:       b.zone_pied,
        zones_corrigees: [...(st.zonesFixed || [])],
        note:            st.note || null,
        corrigee_par:    st.operateur || null,
        statut,
      };
      ...
      setFixSaving(p => { const n = new Set(p); n.delete(b.id); return n; });
    }
  };
      ...
      <div className="card">
        ...
        <div className="home-actions">
          <div className="home-action-row">
            ...
            <button className="action-btn ab-fix" onClick={()=>setScreen("corriger")}>
              <div className="ab-icon">🔧</div>
              <div className="ab-label" style={{color:"var(--red)"}}>CORRIGER</div>
              <div className="ab-sub">Bouteilles en échec</div>
            </button>
          </div>
      ...
      <div className="center-body">
        <div className="form-card">
          ...
          <div className="prog-wrap">
            <div className="prog-bar" style={{width:`${serials.filter(s=>s.trim()).length/BATCH_SIZE*100}%`}}/>
          </div>
